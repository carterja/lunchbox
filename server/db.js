...
const insertRestaurant = db.prepare(`
  INSERT INTO restaurants (id, name, ordering_url, food_type, tags, date_added)
  VALUES (@id, @name, @ordering_url, @food_type, @tags, @date_added)
`);
const deleteRestaurantById = db.prepare('DELETE FROM restaurants WHERE id = ?');
const updateRestaurantById = db.prepare(`
  UPDATE restaurants
  SET name = @name, ordering_url = @ordering_url, food_type = @food_type, tags = @tags
  WHERE id = @id
`);
